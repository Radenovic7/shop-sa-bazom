...
<?php
  ...
  if(isset($_POST["submit-registration"])){
    ...
    $sql="INSERT INTO users(username,password,email,name,surname,address,timestamp) VALUE (:username, :password, :email, :name, :surname, :address, NOW())";
    $statement=$pdo->prepare($sql);
    $statement->execute(["username"=> "$username", "password"=>"$password", "email"=> "$email", "name"=> "$name", "surname"=> "$surname", "address"=> "$address", ]);
  }
  //ISPIS SVIH USERA
